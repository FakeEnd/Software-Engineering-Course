...
def reader_function(path):
    ...
    for file_path in path:
        ...
        @delayed(pure=True)
        def get_tile_model_results(level, column, row):
            tile = dz.get_tile(level, (column, row))
            tile = np.array(tile).transpose((1, 0, 2))
        ...
        for level in range(dz.level_count - 1, -1, -1):  # Reverse to ensure smallest to largest
            ...
            layer_model = da.concatenate(
                [
                    da.concatenate(
                        [
                            da.from_delayed(
                                get_tile_model_results(
                                    level, col, row
                                ),
                                (*sample_tile_shape[:2], 4),
                                np.float32
                            )
                            for row in rows
                        ],
            ...
            layers_model.append(layer_model)
            
        pyramid_layers.append(layers)
        pyramid_layers_hed.append(layers_hed)
        pyramid_layers_model.append(layers_model)
    ...
    layer_data = [
        ...
        (layers_model, {'name':'pyramid_layers_model', 'multiscale': True, 'contrast_limits': [0, 255]}, 'image')
        for layers_model in pyramid_layers_model
    ]
    return layer_data
